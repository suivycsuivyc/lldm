...
class BaseTrainer():
    ...
    def __init__(self, rank, cfg):
        ...
        if self.distributed:
            ...
            dist.init_process_group(backend='nccl', init_method=dist_url, world_size=self.world_size, rank=rank)
            dist.barrier()
    ...
    def make_datasets(self):
        ...
        self.datasets = dict()
        self.loaders = dict()
        self.loader_epoch_setters = dict()
        ...
        for split, spec in cfg.datasets.items():
            ...
            if isinstance(dataset, wds.WebDataset):
                self.log(f'Datasets - {split}: WebDataset')
                self.loaders[split] = wds.WebLoader(dataset, batch_size=None, shuffle=False, num_workers=loader_spec.num_workers // self.world_size)
                self.loader_epoch_setters[split] = None
            else:
                ...
                self.loaders[split], self.loader_epoch_setters[split] = self.make_distributed_loader(
    ...
    def make_model(self, model_spec=None):
        if model_spec is None:
            ...
            model = models.make(model_spec, load_sd=(model_spec.get('sd') is not None))
    ...
    def make_optimizers(self):
        self.optimizers = {'all': utils.make_optimizer(self.model.parameters(), self.cfg.optimizers['all'])}
    ...
    def run_training(self):
        ...
        self.train_loader_epoch_setter = self.loader_epoch_setters['train']
        ...
        for epoch in range(start_epoch, max_epoch + 1):
            ...
            if self.distributed:
                for epoch_setter in self.loader_epoch_setters.values():
                    if epoch_setter is not None and epoch_setter is not self.train_loader_epoch_setter:
            ...
            if self.is_master and (epoch == start_epoch or self.cfg._env.verbose):
            ...
            for _ in pbar:
                ...
                try:
                    ...
                    if self.distributed and self.train_loader_epoch_setter is not None:
                        self.train_loader_epoch_setter.set_epoch(self.train_loader_epoch)
    ...
    def evaluate(self):
        ...
        if self.is_master and self.cfg._env.verbose:
